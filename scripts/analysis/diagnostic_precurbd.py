...
### Diagnostic pre-CURBD.
### Ici on regarde si les signaux ont l'air entrainables avant de cramer une nuit de calcul.
...
datasets = [
    ### Quelques souris de reference pour comparer la qualite du preprocessing.
    (3, 6, 316),
    (3, 6, 322),
    (9, 6, 415),
    (9, 6, 410),
...
pvar_curbd_reference = {
    ### Scores connus/observes: ca donne un point de comparaison rough.
    316: 0.234,
    322: 0.442,
    415: 0.052,
    410: 0.397,
...
def safe_mean(x):
    ### Moyenne qui ignore les NaN. Pas de panique si une metrique est vide.
...
def safe_median(x):
    ### Mediane robuste, meme principe: les NaN restent sur le banc.
    x = np.asarray(x, dtype=float)
    x = x[np.isfinite(x)]
...
def robust_amplitude_per_trace(ts):
    ### Amplitude robuste P95-P5: moins fragile qu'un max-min trop dramatique.
    return (
...
def rms_per_trace(ts):
    ### RMS par trace: energie moyenne du signal.
    return np.sqrt(np.nanmean(ts ** 2, axis=1))


def mean_pairwise_correlation(ts):
    ### Correlation moyenne entre regions: si c'est trop haut, GSR/preprocessing merite un regard.
    corr = np.corrcoef(ts)
...
def median_pairwise_correlation(ts):
    ### Version mediane pour ne pas laisser quelques paires weird dominer.
    corr = np.corrcoef(ts)
    mask = ~np.eye(corr.shape[0], dtype=bool)
    values = corr[mask]
    values = values[np.isfinite(values)]
...
def persistence_pvar_one_frame(ts):
    ### Baseline naive: predire la prochaine frame avec la frame precedente.
    y_true = ts[:, 1:]
    y_pred = ts[:, :-1]
...
def global_signal_variance(ts):
    ### Variance du signal global: indicateur direct du "tout bouge ensemble".
    return float(np.nanvar(np.nanmean(ts, axis=0)))


def gsr_variance_removed_fraction(ts_before, ts_after):
    ### Combien de variance la regression globale a enleve par trace.
...
def compute_stage_metrics(ts, stage, cohort, month, mouse, n_subregions, pvar_curbd):
    ### Calcule un paquet de metriques pour une etape du preprocessing.
    ### Le but: voir a quelle etape le signal devient meilleur ou part en vrille.
    ...
    lag_times, mean_acf, _ = compute_mean_autocorrelation(
        ### Memoire temporelle moyenne du signal.
        ts,
        dt=dtData,
    ...
    ### PSD + puissance cumulee: pour voir ou vit l'energie frequencielle.
    ...
    row = {
        ### Une ligne par souris et par stage: signal recu, GSR, final, etc.
        "cohort": cohort,
        "month": month,
        "mouse": mouse,
...
for cohort, month, mouse in datasets:
    ...
    ts_signal = extract_timeseries_du_tenseur(gcamp, masque_sub)
    ts_signal = np.asarray(ts_signal, dtype=float)

    row_signal, curves_signal = compute_stage_metrics(
        ts_signal, "signal_deja_dff", cohort, month, mouse, n_subregions,
        pvar_curbd_reference.get(mouse, np.nan),
    )
    rows.append(row_signal)
    all_curves[(mouse, "signal_deja_dff")] = curves_signal

    ts_gsr = regress_out_global_signal(ts_signal)
    row_gsr, curves_gsr = compute_stage_metrics(
        ts_gsr, "signal_deja_dff_gsr", cohort, month, mouse, n_subregions,
        pvar_curbd_reference.get(mouse, np.nan),
    )

    gsr_removed = gsr_variance_removed_fraction(ts_signal, ts_gsr)
    row_gsr["gsr_variance_removed_mean"] = safe_mean(gsr_removed)
    row_gsr["gsr_variance_removed_median"] = safe_median(gsr_removed)

    rows.append(row_gsr)
    all_curves[(mouse, "signal_deja_dff_gsr")] = curves_gsr
...
gsr_df = df[df["stage"] == "signal_deja_dff_gsr"].copy()
