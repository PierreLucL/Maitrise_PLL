"""Comparaison appariée des indices tt et tt-1 à durée identique, avec cartes par unité et PCA–CCA.
...
from shared_signal_specificity import cca_test
...
def validate_pair(a, b):
    checked = []
    snapshot=Path(__file__).resolve().parents[2]/'results/narval_paired_controls_20260913/snapshot'
    core='src/maitrise_curbd/curbd.py'
    old=(snapshot/'legacy'/core).read_text()
    new=(snapshot/'corrected'/core).read_text()
    before='r_slice = RNN[iTarget, tt].reshape(number_learn, 1)'
    after='r_slice = RNN[iTarget, tt-1].reshape(number_learn, 1)'
    assert old.count(before)==1 and old.replace(before,after)==new
    for d,variant in [(a,'legacy'),(b,'corrected')]:
        for path,digest in d['reproducibility']['source_sha256'].items():
            assert sha256(snapshot/variant/path)==digest, path
    assert a['parameters']==b['parameters']
    assert a['model_parameters']==b['model_parameters']
    checked.append('Sources vérifiées contre snapshots : seule la ligne r_slice tt → tt-1 diffère ; durée identique')
    ...
    for key in ['timeseries_sha256', 'packages', 'threads']:
...
def main():
    ...
    for ti in range(len(names)):
        for si in range(len(names)):
            ...
            for sj in range(len(names)):
                ...
                held_all=cca_test(x,y,fit,test)
                matches.append(dict(source=sj,test_cca=held,test_cca_all=held_all,test_cca_mean=float(np.mean(held_all)),test_cca_2_10=float(np.mean(held_all[1:])),norm_r=correlation(np.linalg.norm(x[test],axis=1),np.linalg.norm(y[test],axis=1))))
            own=matches[si];wrong=[m for m in matches if m['source']!=si]
            controls.append(dict(target=ti,source=si,matches=matches,own=own,
                                 cca_2_10_margin=own['test_cca_2_10']-max(m['test_cca_2_10'] for m in wrong),
    ...
    meta=dict(files=[str(args.a.resolve()),str(args.b.resolve())],sha256=hashes,
              ...
              limitations='Une graine par souris ; comparaison entre indices, aucune mesure de reproductibilité entre graines du nouvel indice. Réseau entraîné sur toute la session. CCA1 ne garantit ni signe ni amplitude. Pas de preuve biologique ou causale ; pas de reproduction vérifiée ligne par ligne des scripts des auteurs.')
