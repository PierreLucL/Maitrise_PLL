...
### Benchmark autoregressif simple.
### Question: est-ce qu'une trace se predit deja bien avec son propre passe?
### Si oui/non, ca donne un baseline avant d'interpreter CURBD comme un champion.
...
# ============================================================
### PARAMETRES: dataset fixe + grille lag/ridge a tester.
# ============================================================
...
### Chaque trace est predite a partir de SES PROPRES valeurs passees.
### A 12 Hz, lag=1 -> 0.083 s, lag=12 -> 1 s, etc.
lags_list = [
    1,
    2,
    4,
...
### Ridge lambda: 0 = regression normale, plus gros = modele plus sage.
ridge_list = [
    0.0,
    1e-6,
    1e-4,
...
# ============================================================
### OUTILS: petites fonctions de regression, rien de deep learning ici.
# ============================================================
...
def make_ar_design_matrix(trace, n_lags):
    ...
    if trace.ndim != 1:
        ### Ici on veut une seule trace a la fois, pas la matrice complete.
    ...
    ### X contient les valeurs passees; y contient la valeur a predire.
    X = np.column_stack(
        [
...
def fit_ridge_closed_form(X, y, ridge_lambda):
    ...
    ### Standardisation basee uniquement sur le train. Pas de leakage, on reste propre.
    ...
    if ridge_lambda > 0:
        ### Penalite ridge sur les coefficients, mais pas sur l'intercept.
        A = A + ridge_lambda * np.eye(p)

    b = Xz.T @ yz

    try:
        ### Solve direct si possible; pinv en fallback si la matrice est mal conditionnee.
...
def predict_ridge(model, X):
    ### On applique la meme standardisation que pendant le fit. Sinon c'est pas le meme monde.
    X = np.asarray(X, dtype=float)
...
def pvar_score(y_true, y_pred):
    ...
    ### pVar = 1 - erreur relative a la variance des vraies valeurs.
    ss_res = np.sum(
        (y_true - y_pred) ** 2
    )
...
### Les fichiers GCaMP recus sont deja en DeltaF/F. Pas de recalcul de cette etape ici.
