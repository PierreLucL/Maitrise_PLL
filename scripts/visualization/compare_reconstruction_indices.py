"""Traces appariées : cible sauvegardée, RNN tt et RNN tt-1, 100 passages.

Une unité par région, choisie près de la pVar médiane du modèle tt sur toute la
session, avant examen du gain avec tt-1. Aucun lissage ou z-score ajouté.
"""
import argparse
import json
import sys
...
def main():
    ...
    for name,ids in a['regions']:
        ...
        selected.append(dict(region=str(name),unit=unit,median_unit_pvar_tt=med,pvar_tt=float(scores[0][unit]),pvar_tt_minus_1=float(scores[1][unit])))
    ...
    for row,item in enumerate(selected):
        ...
        for col in range(3):
            ...
            if col in [0,1]:
                ax.text(.015,.95,f"pVar unité, session : {item['pvar_tt' if col==0 else 'pvar_tt_minus_1']:.3f}",transform=ax.transAxes,va='top',fontsize=9,bbox=dict(facecolor='white',alpha=.8,edgecolor='none',pad=2))
            if row==len(selected)-1:ax.set_xlabel('Temps (s)')
            else:ax.tick_params(labelbottom=False)
    for ax,title in zip(axes[0],['Indice tt + cible','Indice tt-1 + cible','Superposition · zoom 75–90 s']):ax.set_title(title,pad=28,fontsize=12)
    fig.suptitle(f'Reconstruction des mêmes unités · {mouse} · graine 2026 · 100 passages\nUne unité proche de la pVar médiane du modèle tt par région · mêmes échelles sur chaque ligne',fontsize=15)
    handles=[Line2D([],[],color=target_color,lw=2,label='Cible traitée sauvegardée (Adata)'),Line2D([],[],color=colors[0],ls='--',lw=1.5,label='RNN · tt'),Line2D([],[],color=colors[1],lw=1.5,label='RNN · tt-1')]
    ...
    fig.savefig(out/'reconstruction_indices.png',dpi=180)
    fig.savefig(out/'reconstruction_indices.pdf')
    plt.close(fig)
    metadata=dict(files=comparison['files'],sha256=comparison['sha256'],selection='Unité la plus proche de la pVar médiane de sa région à 100, calculée sur toute la session ; égalités départagées par ordre des unités. Aucun critère basé sur le gain avec tt-1.',
