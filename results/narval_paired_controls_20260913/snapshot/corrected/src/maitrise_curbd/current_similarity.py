...
### Comparaison de courants CURBD entre deux fichiers .pkl.
### C'est le module "est-ce que ces deux runs racontent la meme affaire ou pas pantoute?".
...
def compare_current_pkls(
    ...
    *,
    ...
    ### On ramene les deux fichiers au meme format: liste de 36 courbes.
    ...
    ### Une matrice 6x6 par metrique, mais on calcule d'abord en vecteur de 36 pour garder ca simple.
    out = {name: np.full(36, np.nan, dtype=float) for name in metrics}

    for idx, (curve_a, curve_b) in enumerate(zip(curves_a, curves_b)):
        ### Chaque paire source/cible est comparee independamment.
        a = _as_1d_float_array(curve_a)
        b = _as_1d_float_array(curve_b)

        if resample and a.size != b.size:
            ### Si les runs n'ont pas le meme nombre de points, on remet b sur la grille de a.
        ...
        ### On compare seulement les points finis; les NaN ne votent pas.
        a = a[valid]
        b = b[valid]
...
### Charge un pkl peu importe lequel des formats historiques il utilise.
def _load_36_curves(path: str | Path) -> list[np.ndarray]:
    with Path(path).open("rb") as f:
        data = pickle.load(f)

    if isinstance(data, dict):
        ### Format recent: currents_curves dans un dict.
        if "currents_curves" in data:
            data = data["currents_curves"]

        if _has_target_source_keys(data):
            ### Format dict avec cles (target, source), notre vieux classique.
            return [
    ...
    ### Format array 6x6xT ou 36xT: on a deja vu les deux dans la vraie vie.
    if arr.shape[:2] == (6, 6):
...
### Verifie si un dict contient toutes les paires target/source attendues.
...
### Convertit une courbe en 1D float, parce que les pkls aiment etre creatifs.
...
### Resample lineairement une courbe pour matcher la longueur d'une autre.
...
### Table des metriques disponibles. Ajoute une fonction ici et elle devient callable.
...
### Similarite Pearson: bon pour la forme, sensible aux courbes plates.
...
### Similarite cosinus: regarde l'angle entre deux courbes, pas leur amplitude brute.
...
### Erreur quadratique moyenne, version directe.
def _rmse(a: np.ndarray, b: np.ndarray) -> float:
    return float(np.sqrt(np.mean((a - b) ** 2)))


### RMSE normalisee par l'amplitude de reference pour comparer des courants de tailles differentes.
...
### Erreur absolue moyenne, moins dramatique que RMSE face aux gros outliers.
def _mae(a: np.ndarray, b: np.ndarray) -> float:
    return float(np.mean(np.abs(a - b)))


### Pire erreur absolue: utile pour spotter le "ouch" maximal.
