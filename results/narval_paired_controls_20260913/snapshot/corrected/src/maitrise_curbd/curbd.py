...
### Coeur CURBD/RNN. Code plus algorithmique ici: on garde des commentaires de checkpoints,
### parce qu'une loop RLS de 2h du matin, ca se relit pas comme une recette de Kraft Dinner.
...
def trainMultiRegionRNN(activity, dtData=1, dtFactor=1, g=1.5, tauRNN=0.01,
    ...
    seed: int or None
        Graine locale du RNN (0 a 2**32-1). None conserve le comportement historique.
    """
    if dtData is None:
    ...
    ### Une graine explicite ne modifie pas le generateur global des autres scripts.
    if seed is not None:
        if isinstance(seed, (bool, np.bool_)) or not isinstance(seed, (int, np.integer)) or not 0 <= seed < 2**32:
            raise ValueError('seed doit etre un entier entre 0 et 2**32-1, ou None.')
        seed = int(seed)
    rng = npr if seed is None else np.random.RandomState(seed)

    ### Une unite RNN par trace/region. Simple mental model: chaque ligne de activity devient un neurone.
    ...
    ### Setup du temps et de l'echantillonnage: data a sa grille, RNN a sa grille plus fine.

    learnList = rng.permutation(number_units)
    ...
    iWN = ampWN * rng.randn(number_units, len(tRNN))
    inputWN = np.ones((number_units, len(tRNN)))
    for tt in range(1, len(tRNN)):
        ### Bruit filtre dans le temps: assez pour pousser le systeme, pas juste du white noise sec.
        inputWN[:, tt] = iWN[:, tt] + (inputWN[:, tt - 1] - iWN[:, tt])*np.exp(- (dtRNN / tauWN))
    inputWN = ampInWN * inputWN

    ### Matrice d'interactions initiale: random, scalee par g pour controler le regime.
    J = g * rng.randn(number_units, number_units) / math.sqrt(number_units)
    J0 = J.copy()

    ### Normalisation dans [-0.999, 0.999], sinon arctanh/tanh partent en mode drama.
    Adata = activity.copy()
    activity_scale = Adata.max()
    Adata = Adata/activity_scale
    Adata = np.minimum(Adata, 0.999)
    Adata = np.maximum(Adata, -0.999)

    ### Reference de variance pour calculer pVar plus tard.
    stdData = np.std(Adata[iTarget, :])

    ### Pour comparer RNN et data, on mappe chaque point data au point RNN le plus proche.
    iModelSample = np.zeros(len(tData), dtype=np.int32)
    for i in range(len(tData)):
        iModelSample[i] = (np.abs(tRNN - tData[i])).argmin()

    ### Memoire des trajectoires et des scores: c'est ce qu'on va inspecter apres.
    RNN = np.zeros((number_units, len(tRNN)))
    chi2s = []
    pVars = []

    ### Matrice RLS: le coeur de l'apprentissage, aka le bout qui fait le gros du job.
    ...
    ### Grande loop d'entrainement: nRunTrain apprend, nRunFree laisse rouler pour voir si ca tient.
    for nRun in range(0, nRunTot):
        H = Adata[:, 0, np.newaxis]
        RNN[:, 0, np.newaxis] = nonLinearity(H)
        ### Track quand updater J: le RNN avance plus finement que les frames de donnees.
        tLearn = 0  # keeps track of current time
        iLearn = 0  # keeps track of last data point learned
        chi2 = 0.0

        for tt in range(1, len(tRNN)):
            ### On avance le temps interne du RNN.
            tLearn += dtRNN
            ### Reset optionnel si un jour on colle plusieurs essais ensemble.
            if tt in resetPoints:
                timepoint = math.floor(tt / dtFactor)
                H = Adata[:, timepoint]
            ### Dynamique recurrente: tanh(H), courant J*R, puis integration Euler.
            RNN[:, tt, np.newaxis] = nonLinearity(H)
            JR = (J.dot(RNN[:, tt]).reshape((number_units, 1)) +
                  inputWN[:, tt, np.newaxis])
            H = H + dtRNN*(-H + JR)/tauRNN
            ### Quand on retombe sur un temps data, on compare et on corrige J.
            if tLearn >= dtData:
                tLearn = 0
                err = RNN[:, tt, np.newaxis] - Adata[:, iLearn, np.newaxis]
                iLearn = iLearn + 1
                ### Chi2 garde une trace de l'erreur; notre petit compteur de souffrance.
                chi2 += np.mean(err ** 2)

                if nRun < nRunTrain:
                    ### Update RLS: c'est ici que J apprend vraiment a coller aux donnees.
                    r_slice = RNN[iTarget, tt-1].reshape(number_learn, 1)
                    k = PJ.dot(r_slice)
        ...
        ### Fin d'un run: on score la trajectoire RNN contre la vraie activite.
        ...
        if fig:
            ...
            idx = rng.choice(range(len(iTarget)))
    ...
    ### On emballe tout dans un dict pour que les scripts puissent sauver/analyser sans recalculer.
    out_params = {}
    ...
    out_params['seed'] = seed
    out_params['rng_algorithm'] = 'MT19937 (RandomState)' if seed is not None else 'numpy.random global'
    ...
    out['dtRNN'] = dtRNN
    out['Adata'] = Adata
    out['activity_scale'] = float(activity_scale)
    out['initial_state'] = Adata[:, 0].copy()
    out['tData'] = tData
    out['dtData'] = dtData
    out['J'] = J
    out['J0'] = J0
...
def computeCURBD(sim):
    ...
    ### On ressort les morceaux du modele entraine: trajectoire RNN, matrice J, regions.
    RNN = sim['RNN']
    J = sim['J'].copy()
    regions = sim['regions']
    ...
    if current_type == 'excitatory':  # take only positive J weights
        ### Option excitatrice: on garde juste les poids positifs.
        J[J < 0] = 0
    elif current_type == 'inhibitory':  # take only negative J weights
        ### Option inhibitrice: on garde juste les poids negatifs.
        J[J > 0] = 0
    ...
    ### On calcule tous les courants source -> cible entre regions parentes.
    ...
    for idx_trg in range(nRegions):
        ...
        for idx_src in range(nRegions):
            ### Sous-matrice J cible/source, multipliee par l'activite RNN de la source.
