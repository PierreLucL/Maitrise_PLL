...
titre_du_test = "FAST_screening_n_pixels_CURBD"
...
# Hyperparamètres fixés à partir des screenings précédents
g = 0.8
ampInWN = 0.01
tauRNN = 0.33
dtFactor = 2
...
for i, n_pixels in enumerate(configs):
...
#
# La subdivision en sous-régions dépend de n_pixels et sera
# donc refaite à l'intérieur de chaque configuration.
# ============================================================
...
for i_config, n_pixels in enumerate(configs):
    ...
    try:
        ...
        # ----------------------------------------------------
        # Subdivision spatiale propre à cette configuration
        # ----------------------------------------------------
        ...
        print(
            f"Nombre de sous-régions = {n_subregions}"
        )

        print(
            f"Nombre de régions parentes = {n_parent_regions}"
        )
        ...
        if not np.all(np.isfinite(ts_raw)):
            raise ValueError(
                "ts_raw contient des NaN ou des inf."
            )
        ...
        if not np.isfinite(curbd_scale) or curbd_scale <= 0:
            raise ValueError(
                "Maximum invalide avant normalisation CURBD."
            )
        ...
        print(
            f"Clipping + : {100 * fraction_clip_pos:.6f}% | "
            f"Clipping - : {100 * fraction_clip_neg:.6f}%"
        )
        ...
        row["n_subregions"] = int(ts_preprocessed.shape[0])
        row["n_parent_regions"] = int(len(regions))
        row["n_regions"] = int(len(regions))
        ...
        row["n_subregions"] = ts.shape[0]
        row["n_parent_regions"] = len(regions)
        ...
        to_save = {
            ...
            "parameters": {
                ...
                "mouse": souris,
                "n_pixels": n_pixels,
                "n_subregions": int(ts.shape[0]),
                "n_parent_regions": int(len(regions)),
                "fps": 1 / dtData,
                "dtData": dtData,
                "dtFactor": dtFactor,
                "dtRNN": dtData / dtFactor,
...
if not done_df.empty:
    valid = done_df[
        np.isfinite(done_df["pVar_free_mean"])
    ].copy()
    ...
    if not valid.empty:
        ...
        for rank, (_, best) in enumerate(
            ...
            print(
                f"{rank}. "
                f"n_pixels={int(best['n_pixels'])} | "
                f"n_subregions={int(best['n_subregions'])} | "
                f"pVar libre={best['pVar_free_mean']:.4f} | "
                f"chi2 libre={best['chi2_free_mean']:.4f} | "
                f"runtime={best['runtime_sec']/60:.1f} min"
            )
